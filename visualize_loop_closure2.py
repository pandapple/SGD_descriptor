...
def parse_result_file(filename: str) -> Tuple[Dict[int, np.ndarray], List[Tuple[int, int, float, Optional[int]]]]:
    ...
    Returns:
        ...
        loop_closures: 列表，[(frame_id1, frame_id2, match_score, is_correct_or_none), ...]
    """
    trajectory = {}
    loop_closures = []
    ...
    for line in lines:
        ...
        if section == 'trajectory':
            ...
            # 解析回环数据：
            # - old: frame_id1 frame_id2 [match_score] [pose_distance]
            # - new: frame_id1 frame_id2 [match_score] [pose_distance] is_correct(1/0)
            parts = line.split()
            if len(parts) >= 2:
                try:
                    ...
                    is_correct = None
                    # If the last token is 0/1, treat it as correctness flag.
                    if len(parts) >= 3:
                        last = parts[-1]
                        if last in ('0', '1'):
                            is_correct = int(last)
...
def visualize_3d(trajectory: Dict[int, np.ndarray], 
    ...
    # 轨迹偏移方向：仅沿 z 轴（向上）
    offset_vector = np.array([0.0, 0.0, offset_distance])
    ...
    if loop_closures:
        ...
        for frame_id1, frame_id2, score, is_correct_flag in loop_closures:
            if frame_id1 in trajectory and frame_id2 in trajectory:
                ...
                # 判断匹配是否正确：
                # - preferred: use last-column flag from result file
                # - fallback: use distance threshold for old files
        ...
        if any(flag is not None for *_, flag in loop_closures):
            print(f"  Correct matches (by is_correct flag): {correct_matches}")
            print(f"  Incorrect matches (by is_correct flag): {incorrect_matches}")
        else:
...
Examples:
  ...
  python visualize_loop_closure.py result.txt --show-frame-ids
    ...
    parser.add_argument('--offset-distance', type=float, default=100.0,
    ...
    parser.add_argument('--distance-threshold', '-d', type=float, default=20.0,
                       help='Fallback distance threshold for old result files without is_correct column')
